...
//	queue_event_t evt;	
	...
	// Портируемые ISR-safe функции вместо прямого доступа к регистрам
	// TIMERG1 (структура регистров Timer Group на S3 отличается от ESP32).
	timer_group_intr_clr_in_isr(TIMERGROUP1MS, timer_idx); //isr ack
#else
	...
	if (serviceAddon != NULL) serviceAddon(); // for the encoders and buttons
#ifdef CONFIG_IDF_TARGET_ESP32S3
	timer_group_enable_alarm_in_isr(TIMERGROUP1MS, timer_idx);
#else
...
void   sleepCallback(void *pArg) {
	...
	queue_event_t evt;		
#ifdef CONFIG_IDF_TARGET_ESP32S3
	timer_group_intr_clr_in_isr(TIMERGROUP, timer_idx); //isr ack
#else
	...
	xQueueSendFromISR(event_queue, &evt, NULL);	
#ifdef CONFIG_IDF_TARGET_ESP32S3
	// Таймер одноразовый (auto_reload выключен) — портируемо просто
	// останавливаем счётчик вместо прямого сброса alarm_en.
	timer_group_set_counter_enable_in_isr(TIMERGROUP, timer_idx, TIMER_PAUSE);
#else
...
void   wakeCallback(void *pArg) {
	...
	queue_event_t evt;	
#ifdef CONFIG_IDF_TARGET_ESP32S3
	timer_group_intr_clr_in_isr(TIMERGROUP, timer_idx);
#else
...
// ============================================================
// АВТОПОДКЛЮЧЕНИЕ К ДОМАШНЕЙ WiFi-СЕТИ
// Замените SSID и пароль на свои!
// ============================================================
#define MY_WIFI_SSID     "sergey"      // <-- ВАШ SSID
#define MY_WIFI_PASSWORD "serhii1883"           // <-- ВАШ ПАРОЛЬ

if (strlen(g_device->ssid1) == 0)
{
    ESP_LOGI(TAG, "=== First run: writing WiFi credentials ===");
    strncpy(g_device->ssid1, MY_WIFI_SSID, SSIDLEN - 1);
    g_device->ssid1[SSIDLEN - 1] = '\0';
    strncpy(g_device->pass1, MY_WIFI_PASSWORD, PASSLEN - 1);
    g_device->pass1[PASSLEN - 1] = '\0';
    g_device->dhcpEn1 = 1;
    g_device->current_ap = STA1;
    saveDeviceSettings(g_device);
    ESP_LOGI(TAG, "WiFi saved: %s", g_device->ssid1);
}
else
{
    ESP_LOGI(TAG, "WiFi already configured: %s", g_device->ssid1);
}
// ============================================================
...
// lcd init
	uint8_t rt = 0;
	uint8_t saved_lcd_type = 0;

	option_get_lcd_info(&saved_lcd_type, &rt);
	
	if (saved_lcd_type != 0 && saved_lcd_type != 255) {
		g_device->lcd_type = saved_lcd_type;
	}

	ESP_LOGI(TAG, "Инициализация дисплея из NVS. Тип: %d, Ротация: %d", g_device->lcd_type, rt);
	...
	xTaskCreatePinnedToCore(timerTask, "timerTask",3000, NULL, PRIO_TIMER, &pxCreatedTask,CPU_TIMER); 
	ESP_LOGI(TAG, "%s task: %x","t0",(unsigned int)pxCreatedTask);		
	
	xTaskCreatePinnedToCore(uartInterfaceTask, "uartInterfaceTask", 3500, NULL, PRIO_UART, &pxCreatedTask,CPU_UART); 
...
//-----------------------------------------------------
...

	{
		...
		if (usb_en) usb_msc_init(); // MVP: USB flash drive (MSC) playback support
		else ESP_LOGI(TAG, "USB flash drive playback disabled in settings, not starting USB host");
	}
	...
	vTaskDelay(1);
	xTaskCreatePinnedToCore(clientTask, "clientTask", 9000, NULL, PRIO_CLIENT, &pxCreatedTask,CPU_CLIENT); 
	ESP_LOGI(TAG, "%s task: %x","clientTask",(unsigned int)pxCreatedTask);	
	vTaskDelay(1);
    xTaskCreatePinnedToCore(serversTask, "serversTask", 5000, NULL, PRIO_SERVER, &pxCreatedTask,CPU_SERVER); 
	ESP_LOGI(TAG, "%s task: %x","serversTask",(unsigned int)pxCreatedTask);	
	vTaskDelay(1);
	xTaskCreatePinnedToCore (task_addon, "task_addon", 4000, NULL, PRIO_ADDON, &pxCreatedTask,CPU_ADDON);  
