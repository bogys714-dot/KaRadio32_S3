...
#include "eeprom.h"
//4400
// Was 5400. The big single-buffer stack consumers (the 1536-byte request
// buffer, and the /instant_play, /hardware, usbfiles response buffers) have
// been moved to the heap (see webserver.c), so this is now a safety margin
// rather than the only thing standing between normal use and an overflow.
#define stack  	6144 
#define TAG	"Servers"
