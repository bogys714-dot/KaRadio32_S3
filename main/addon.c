...
// list of screen
typedef  enum typeScreen {smain,svolume,sstation,snumber,stime,susb,snull} typeScreen ;
...
Button_t* usbButton = NULL;   // dedicated single-pin Radio<->USB button
static bool isUsbButton = true;
...
////////////////////
// draw the USB file list screen (browse/select). Same visual pattern as
// drawStation() -- one file name shown at a time, "index/total" as the
// number -- just backed by the USB playlist instead of the station list.
static int usbListIndex = 0;

void drawUsbList()
{
	int count = usb_msc_playlist_count();
	char sNum[32];
	char ddot[96];

	if (count <= 0) {
		snprintf(sNum, sizeof(sNum), "-");
		snprintf(ddot, sizeof(ddot), "No files");
	} else {
		if (usbListIndex < 0) usbListIndex = count - 1;
		if (usbListIndex >= count) usbListIndex = 0;
		snprintf(sNum, sizeof(sNum), "%d/%d", usbListIndex + 1, count);
		const char *name = usb_msc_playlist_name(usbListIndex);
		snprintf(ddot, sizeof(ddot), "%s", name ? name : "?");
	}

	if (lcd_type != LCD_NONE)
		isColor?drawUsbListUcg(mTscreen,sNum,ddot):drawUsbListU8g2(mTscreen,sNum,ddot);
}

// Move the highlighted file up/down (wrap), called only while stateScreen==susb
static void changeUsbList(int16_t value)
{
	int count = usb_msc_playlist_count();
	if (count <= 0) return;
	if (value > 0) usbListIndex++;
	else if (value < 0) usbListIndex--;
	if (usbListIndex >= count) usbListIndex = 0;
	if (usbListIndex < 0) usbListIndex = count - 1;
}

static void evtUsbList(int16_t value)
{ // value +1 or -1
	event_lcd_t evt; 
	evt.lcmd = eusblist;
...
// Play whatever is currently highlighted in the USB browse screen, then
// leave the browse screen (back to the normal "now playing" display).
static void confirmUsbSelection(void)
{
	int count = usb_msc_playlist_count();
	if (count <= 0) return;
	const char *name = usb_msc_playlist_name(usbListIndex);
	if (!name) return;
	char path[192];
	snprintf(path, sizeof(path), "/usb0/%s", name);
...
// Toggle Internet radio <-> USB playback (called from the dedicated USB button)
static void toggleRadioUsb(void)
{
	ESP_LOGI("addon", "toggleRadioUsb");
	if (stateScreen == susb) {
		// Already browsing the list: click again = cancel, back to radio/main
		Screen(smain);
		wakeLcd();
		return;
	}
	if (usb_msc_is_playing()) {
		// USB -> Radio
		usb_msc_request_stop();
		startStation();
	} else {
		// Radio -> open the USB file list so a track can be picked
		if (!usb_msc_is_ready()) {
			ESP_LOGW("addon", "USB not ready — insert flash?");
			return;
		}
		if (usb_msc_playlist_count() <= 0) {
			ESP_LOGW("addon", "USB playlist empty");
			return;
		}
		usbListIndex = usb_msc_playlist_current_index();
		if (usbListIndex < 0) usbListIndex = 0;
		Screen(susb);
		wakeLcd();
	}
}
...
{	
	...
	if (newButton == Clicked) {
		if (stateScreen == susb) {
			confirmUsbSelection(); // pick highlighted file & play it
		} else {
			startStop();
		}
	}
	else if (newButton == DoubleClicked) {
		toggletime();
	}
	else if (newButton == Held) {
		// Hold + rotate → station list (original KaRadio)
		if ((getPinState(enc) == getpinsActive(enc)) && (newValue != 0)) {
			if (stateScreen != (role ? sstation : svolume))
				role ? evtStation(newValue) : setRelVolume(newValue);
		}
	}

	// --- rotation without button (volume / stations) — original ---
	{
		// Browsing the USB file list: plain rotation moves the highlight.
		if (stateScreen == susb) {
			if (newValue != 0) evtUsbList(newValue);
			return;
		}

		// While a USB file is playing (and not browsing the list), plain
		// rotation (no button held) skips/rewinds tracks instead of
		// adjusting volume. Hold+rotate (above) is untouched, still does
		// the station list either way.
		if (usb_msc_is_playing() && newValue != 0) {
			if (newValue > 0) usb_msc_play_next();
			else usb_msc_play_prev();
			return;
		}

		typeScreen estate = snull;
		if ((isEncoder0 ^ isEncoder1)&& (!isEsplay))
		{
			if (role) estate = sstation; else estate = svolume;
		}
...
// button1 = station control or volume when pushed
	...
	if (isUsbButton) {
		Button usbBtn = getButtons(usbButton, 0);
		if (usbBtn == Clicked) toggleRadioUsb();
	}
	if (!isEsplay)
	{
...
//	struct device_settings *device;
	...
	// Dedicated USB play/stop button (single pin, active-low with pull-up --
	// matches the on-board BOOT button used by default, see PIN_BTN_USB).
	if (PIN_BTN_USB == GPIO_NONE) isUsbButton = false;
	else usbButton = ClickButtonsInit(PIN_BTN_USB, GPIO_NONE, GPIO_NONE, LOW);
...
//	ServiceAddon();
	if (divide++ == 10) // only every 10ms
	{
		...
		if (isUsbButton) serviceBtn(usbButton);
		if (!isEsplay)
		{
...
//			if (lcd_type == LCD_NONE) continue;
			// FIX: was ESP_LOGV(TAG,"event_lcd: %x  %s, mTscreen: %d",...)
			// printing the full (potentially long, Cyrillic) title string
			// on every single lmeta event - extra stack/CPU cost on the
			// task's hottest, most frequent path for no real debugging
			// value beyond what the len is already useful for.
			if (evt.lcmd != lmeta)
				ESP_LOGV(TAG,"event_lcd: %x, %d, mTscreen: %d",(int)evt.lcmd,(int)evt.lline,mTscreen);
			else
				ESP_LOGV(TAG,"event_lcd: %x  len:%d, mTscreen: %d",(int)evt.lcmd,(int)(evt.lline?strlen(evt.lline):0),mTscreen); 
			switch(evt.lcmd)
			{
				case lmeta:
				...
				case eusblist:
					if(xQueuePeek(event_lcd, &evt1, 0))
						if (evt1.lcmd == eusblist) {evt.lline = NULL;break;}
					changeUsbList((int16_t)(uint32_t)evt.lline);
					Screen(susb);
					wakeLcd();
					evt.lline = NULL;	// just a number
					break;
...
void task_addon(void *pvParams)
{
	...
	xTaskCreatePinnedToCore(rmt_nec_rx_task, "rmt_nec_rx_task", 4096, NULL, PRIO_RMT, &pxCreatedTask,CPU_RMT);
	...
	if (g_device->lcd_type!=LCD_NONE)
	{
		...
		// FIX: 2300 bytes left almost no margin once icy4Ucg()/namesetUcg()
		// (each with a 256-byte BUFLEN local buffer), the ucg_* font/width
		// calculations, and ESP_LOGV formatting on the lmeta path are all
		// active in the same call chain - this is what caused the
		// "stack overflow in task task_lcd" crash. Give it real headroom.
		xTaskCreatePinnedToCore (task_lcd, "task_lcd", 4096, NULL, PRIO_LCD, &pxTaskLcd,CPU_LCD); 
