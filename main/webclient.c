...
static void clientSaveMetadata(char* s,int len)
{
	...
	// FIX: 's' can be a pointer straight into the live network receive buffer
	// (see clientReceiveCallback: "clientSaveMetadata(inpdata+metad+1,l)"),
	// which is immediately followed by raw MP3/audio bytes belonging to the
	// next block - there is no guarantee those audio bytes contain a NUL,
	// a "'", or any other character this function searches for. If the
	// station doesn't NUL-terminate exactly at the end of its declared
	// metadata block (or pads with something other than 0x00), every
	// strstr()/strlen() call below can run straight past the block boundary
	// into audio data - producing the garbage/hieroglyph text seen on the
	// display, and in the worst case reading past the end of the receive
	// buffer entirely. Make a bounded, guaranteed NUL-terminated local copy
	// first and do all parsing on that instead of the live buffer.
	char *sbuf = incmalloc(len+1);
	if (sbuf == NULL) return;
	memcpy(sbuf, s, len);
	sbuf[len] = 0;
	s = sbuf;
	...
	if (ilen > len)
	{
		...
		ilen = len;
		// FIX: was t[len+1]=0, which writes one byte past the 'len' bytes this
		// string is supposed to be truncated to - harmless before when 's' was
		// the live network buffer with slack after it, but a real 1-byte heap
		// overflow now that parsing happens on the tightly-sized 'sbuf' above.
		t[len]=0;
	}
	// FIX: the old code guessed where the closing quote of StreamTitle='...'
	// sat by jumping to a position based on 'ilen' (t_end -=3/-1, or
	// t+ilen-2 as a last resort) and searching forward from there. That
	// guess is only valid when the station doesn't pad the metadata block
	// past the real closing quote. When it does pad with non-NUL bytes
	// (garbage/leftover audio bytes instead of zeros), the guessed start
	// position lands deep inside that padding, the search finds nothing
	// (or a stray "'" in the garbage), and the whole padded block - garbage
	// included - is let through as the "title". Search for the real ICY
	// delimiter "';" starting from the BEGINNING of the title text instead,
	// which is exactly where the protocol places it regardless of any
	// padding that follows.
	if (found)
	{
		t_end = strstr(t, "';");
		if (t_end == NULL) t_end = strrchr(t, '\'');	// fallback: bare quote, no semicolon
		if (t_end != NULL)
		*t_end = 0;
		if (t!=NULL)
		{
	...
	{
		// FIX: previously this branch only decremented 'ilen' without ever
		// actually truncating 't' - meaning a block containing NONE of the
		// expected markers (song_spot=, ;StreamUrl=', StreamTitle=') was let
		// through completely untouched, up to the full 256 bytes, straight
		// to the display. A metadata block with no recognizable marker at
		// all isn't a valid title - it's a strong sign the reconstruction
		// upstream grabbed the wrong bytes (e.g. raw audio instead of the
		// real ICY metadata block). Treat it the same as other invalid
		// input: drop it and clear the displayed title rather than pass
		// unvalidated raw bytes through.
		ESP_LOGD(TAG,"clientSaveMetadata: no marker found, discarding len:%d char:%s",ilen,t);
		if (header.members.mArr[METADATA] != NULL)
			incfree(header.members.mArr[METADATA],"metad");
		header.members.mArr[METADATA] = NULL;
		incfree(sbuf,"metabuf");
		return;
	}
	// the expurged str
...
// see if meta is != of the old one
		...
		((header.members.mArr[METADATA] != NULL)&&(t!= NULL)&&(strcmp(tt,header.members.mArr[METADATA]) != 0)))
	{
		...
		{	ESP_LOGV(TAG,strcMALLOC1,"metad");
			incfree(tt,"");			// FIX: was leaked on this early return
			incfree(sbuf,"metabuf");	// FIX: was leaked on this early return
			return;
		}

		strcpy(header.members.mArr[METADATA], t);
//		dump((uint8_t*)(header.members.mArr[METADATA]),strlen(header.members.mArr[METADATA]));
		// FIX: buffer above was allocated as (ilen+3) bytes, matching the actual
		// extracted-title length 'ilen' (== strlen(t)). Passing 'len' here instead
		// - the length of the whole raw ICY metadata block, which can be far larger
		// than the extracted title - made stringify() read past the end of this
		// small buffer into unrelated heap memory. That explains both the garbage/
		// hieroglyph characters appended to track titles, and the heap corruption
		// (TLSF block_trim_free assert) seen when the oversized bogus read/alloc
		// later got trimmed.
		header.members.mArr[METADATA] = stringify(header.members.mArr[METADATA],ilen);
...
//
	...
	incfree(tt,"");
	incfree(sbuf,"metabuf");	// FIX: free the bounded local copy allocated above
}
...
void clientReceiveCallback(int sockfd, char *pdata, int len)
{
	...
	static uint16_t metadataSize = 0;	// allocated size of 'metadata' buffer (fix: needed to bound strcat below)
	uint16_t l =0;
	uint32_t lc;
	char *inpdata;
	char* inpchr;
...
// meta data computing
		if (rest <0)
		{
			...
			if (metadata != NULL)
			{
				// fix: bound the append to the space actually allocated in 'metadata',
				// instead of trusting that pdata always fits (heap overflow otherwise)
				size_t curLen = strlen(metadata);
				if (metadataSize > curLen + 1)
				{
					strncat(metadata, pdata, metadataSize - curLen - 1);
				}
			}
			if (len >= -rest)
			{
...
//printf("Negative len out = %d, pdata: %x,metad= %d  rest= %d \n",len,pdata,metad,rest);
		...
		if ((header.members.single.metaint != 0) && (clen > metad))
		{
			...
			while ((clen > metad) && (header.members.single.metaint != 0)) // in buffer
			{
				...
				// FIX: 'inpdata' is (signed) char*. The ICY metadata-length byte is an
				// UNSIGNED value 0-255 (length = byte*16, per the Shoutcast/Icecast spec).
				// Reading it as a signed char turned any byte >= 128 into a negative
				// number, which -when multiplied by 16 and stored into uint16_t l- wrapped
				// around into a huge bogus length. That corrupted 'rest' and all buffer
				// math that follows, causing heap corruption / random crashes - this was
				// far more likely to happen with longer, multi-byte UTF-8 (e.g. Cyrillic)
				// track titles or station descriptions, since they need more 16-byte
				// blocks and so are more likely to require a length byte >= 128.
				l = (uint16_t)((unsigned char)inpdata[metad]) * 16;	// new meta length
				rest = clen - metad - l - 1;
				...
				if (l != 0)
				{
					...
					if (rest < 0)
					{
						*(inpdata + clen) = 0; //truncated
						...
						// FIX: strcpy trusted the source to be no longer than 'l' bytes.
						// Bound it explicitly to the buffer we just allocated.
						strncpy(metadata, inpdata + metad + 1, metadataSize - 1);
