...
static void respOk(int conn,const char* message)
{
	...
	// This used to be a VLA sized by strlen(message) -- fine while callers
	// only passed short status strings, but a growing risk now that some
	// callers (e.g. usbfiles) can pass back up to ~1KB of JSON: the VLA's
	// size just moves the same stack-overflow risk here instead of fixing
	// it. Heap-allocate instead.
	char *fresp = inmalloc(strlen(strsROK)+strlen(message)+15);
	if (fresp == NULL) {
		ESP_LOGE(TAG," %s kmalloc fails","respOk");
...
// Small helper to send a plain text response for the /ota endpoint.
static void sendOtaResponse(int conn, bool ok, const char* msg)
{
	char resp[192];
	int mlen = strlen(msg);
	snprintf(resp, sizeof(resp),
		"HTTP/1.1 %s\r\nContent-Type: text/plain\r\nContent-Length: %d\r\nConnection: close\r\n\r\n%s",
		ok ? "200 OK" : "500 Internal Server Error", mlen, msg);
	write(conn, resp, strlen(resp));
}

// Handle a firmware upload sent directly by the browser: POST /ota
// The body (the raw .bin) is streamed straight into the inactive OTA
// partition in small chunks, never buffered whole in RAM - unlike the
// generic POST path (see uint16_t cl / buf[DRECLEN] above), which only
// works for small form bodies and cannot hold a multi-hundred-KB image.
// head/headRecbytes/bodyStart: whatever headers (+ maybe first body bytes)
// were already read into the connection buffer before we recognised /ota.
static void handleOtaUpload(int client_sock, char* head, int headRecbytes, char* bodyStart, uint32_t contentLength)
{
	esp_err_t err;
	esp_ota_handle_t ota_handle;
	const esp_partition_t *update_partition = esp_ota_get_next_update_partition(NULL);

	ESP_LOGI(TAG, "OTA upload starting, size: %u bytes, partition: %s",
			 (unsigned)contentLength, update_partition ? update_partition->label : "NULL");

	if ((update_partition == NULL) || (contentLength == 0)) {
		sendOtaResponse(client_sock, false, "No OTA partition / empty upload");
		return;
	}

	err = esp_ota_begin(update_partition, contentLength, &ota_handle);
	if (err != ESP_OK) {
		ESP_LOGE(TAG, "esp_ota_begin failed: %s", esp_err_to_name(err));
		sendOtaResponse(client_sock, false, "esp_ota_begin failed");
		return;
	}

	uint32_t received = 0;

	// Write whatever body bytes arrived together with the headers first.
	int already = headRecbytes - (int)(bodyStart - head);
	if (already > 0) {
		if ((uint32_t)already > contentLength) already = contentLength; // safety clamp
		err = esp_ota_write(ota_handle, bodyStart, already);
		if (err != ESP_OK) {
			ESP_LOGE(TAG, "esp_ota_write (head) failed: %s", esp_err_to_name(err));
			esp_ota_end(ota_handle);
			sendOtaResponse(client_sock, false, "esp_ota_write failed");
			return;
		}
		received = already;
	}

	// Stream the rest directly from the socket into flash.
	#define OTA_CHUNK 2048
	char* chunk = inmalloc(OTA_CHUNK);
	if (chunk == NULL) {
		esp_ota_end(ota_handle);
		sendOtaResponse(client_sock, false, "low memory");
		return;
	}

	bool sockError = false;
	while (received < contentLength) {
		int toread = contentLength - received;
		if (toread > OTA_CHUNK) toread = OTA_CHUNK;
		int r = read(client_sock, chunk, toread);
		if (r < 0) {
			if (errno == EAGAIN) { vTaskDelay(1); continue; }
			ESP_LOGE(TAG, "OTA socket read error errno:%d", errno);
			sockError = true;
			break;
		}
		if (r == 0) { vTaskDelay(1); continue; } // peer still sending
		...
		received += r;
		vTaskDelay(1); // give other tasks (audio!) room to run during the transfer
	}
	infree(chunk);

	if (sockError || (received != contentLength)) {
		esp_ota_end(ota_handle); // discard - do NOT switch boot partition
		sendOtaResponse(client_sock, false, "incomplete upload");
		return;
	}
	...
	if (err != ESP_OK) {
		...
		sendOtaResponse(client_sock, false, "invalid firmware image");
		return;
	}
	...
	if (err != ESP_OK) {
		...
		sendOtaResponse(client_sock, false, "esp_ota_set_boot_partition failed");
		return;
	}

	sendOtaResponse(client_sock, true, "OK, rebooting");
	vTaskDelay(200 / portTICK_PERIOD_MS);
	esp_restart();
}


// Stream the currently playing USB file so the browser Monitoring <audio> can play it.
// Request: GET /usbplay   or   ?usbplay
static void serveUsbPlay(int conn)
{
	const char *path = usb_msc_current_path();
	...
	if (strstr(path, ".aac") || strstr(path, ".m4a") || strstr(path, ".mp4"))
		ctype = "audio/aac";
	...
	while ((n = fread(buf, 1, sizeof(buf), f)) > 0) {
		if (write(conn, buf, n) < 0) break;
		vTaskDelay(1); // yield so local playback is not starved
	}
	fclose(f);
}
...
// Decode a %XX percent-encoded (URL-encoded) string in place, and turn
// '+' into a space -- browsers/JS encode spaces and special characters
// this way (e.g. encodeURIComponent("A B.mp3") -> "A%20B.mp3"), but a
// filename on the USB drive contains the real characters, not "%20".
// Without this, fopen() on a percent-encoded path just fails to find
// the file (looks like "starts playing, then instantly stops").
...
//	int i;
	...
	if(strcmp(name, "/instant_play") == 0) {
		if(data_size > 0) {
			// url+path used to be 768 bytes of stack (256+512) inside this
			// single big function; heap-allocate them instead.
			char *url = inmalloc(256);
			char *path = inmalloc(512);
			char port[10];
			if (url == NULL || path == NULL) {
				ESP_LOGE(TAG," %s kmalloc fails","instant_play");
				infree(url); infree(path);
				respKo(conn);
				return;
			}
...
//printf("data:%s\n",data);
...
} else if(strcmp(name, "/hardware") == 0)
	{
		...
		extern void option_set_usben(uint8_t enca);
		extern void option_get_usben(uint8_t *enca);
		...
		if(data_size > 0) {
			...
			char coutput[6];
			if(getSParameterFromResponse(coutput, 6, "coutput=", data, data_size)) {
				cout = atoi(coutput);
			}
			
			char clcd[6];
			if(getSParameterFromResponse(clcd, 6, "idlcd=", data, data_size)) {
				lcd_type = atoi(clcd);
				printf("Распознан ID дисплея: %d\n", lcd_type);
			}

			char crt[6];
			if(getSParameterFromResponse(crt, 6, "rt=", data, data_size)) {
				lcd_rot = atoi(crt);
			...
			char cusben[6];
			if(getSParameterFromResponse(cusben, 6, "usben=", data, data_size)) {
				usb_en = atoi(cusben);
				got_usben = true;
				printf("Распознан флаг USB: %d\n", usb_en);
			}
			...
			if (val && lcd_type != 91) 
			{
				if (g_device->audio_output_mode != cout || g_device->lcd_type != lcd_type) {
					...
					g_device->lcd_type = lcd_type;
					changed = true;
				}

				option_set_lcd_info(lcd_type, lcd_rot);
				changed = true; 

				if (changed) {
					saveDeviceSettings(g_device);
					printf("Настройки дисплея, аудио и ротации (%d) сохранены в NVS/eeprom!\n", lcd_rot);
				}
			}
			...
			if (val && got_usben) {
				option_set_usben(usb_en);
				printf("USB (%d) сохранён в NVS! Изменение применится после перезагрузки.\n", usb_en);
				changed = true; // needs a reboot to actually start/stop the USB host
			}

			uint8_t dummy_lcd;
			option_get_lcd_info(&dummy_lcd, &lcd_rot);
			option_get_wdaylang(&wday_lang);
			option_get_usben(&usb_en);
			bool usb_ready = usb_msc_is_ready();

			char *resp_safe = inmalloc(320);
			if (resp_safe == NULL) {
				ESP_LOGE(TAG," %s kmalloc fails","hardware");
				respKo(conn);
				return;
			}
			int json_len = sprintf(resp_safe, "{\"coutput\":\"%d\",\"lcd\":\"%d\",\"rt\":\"%d\",\"wdaylang\":\"%d\",\"usben\":\"%d\",\"usbready\":\"%d\"}", g_device->audio_output_mode, g_device->lcd_type, lcd_rot, wday_lang, usb_en, usb_ready?1:0);
			
			int total_len = sprintf(resp_safe, 
				"HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nContent-Length: %d\r\nConnection: close\r\n\r\n"
				"{\"coutput\":\"%d\",\"lcd\":\"%d\",\"rt\":\"%d\",\"wdaylang\":\"%d\",\"usben\":\"%d\",\"usbready\":\"%d\"}", 
				json_len, g_device->audio_output_mode, g_device->lcd_type, lcd_rot, wday_lang, usb_en, usb_ready?1:0);

			write(conn, resp_safe, total_len);
			infree(resp_safe);

			if (changed) {
				printf("Перезагрузка по запросу веб-интерфейса...\n");
				vTaskDelay(200 / portTICK_PERIOD_MS);
				esp_restart();
			}
			return;
...
// usbfile command   ?usbfile=trackname.mp3  play a file from the USB drive
// (mount point /usb0), same decode/render pipeline as internet radio
				param = getParameterFromResponse("usbfile=", c, strlen(c)) ;
				if (param != NULL) {
					url_decode_inplace(param); // "%20" -> ' ', etc.
					char path[280];
					snprintf(path, sizeof(path), "/usb0/%s", param);
					usb_msc_play_file_async(path);
					infree(param);
					return true;
				}
// usbhalt command   ?usbhalt  stop USB file playback
				param = strstr(c,"usbhalt") ;
				if (param != NULL) {
					usb_msc_request_stop();
					respOk(conn,NULL);
					return true;
				}
// reboot command   ?reboot  soft restart (e.g. if USB did not mount)
				param = strstr(c,"reboot");
				if (param != NULL) {
					respOk(conn,"rebooting");
					vTaskDelay(pdMS_TO_TICKS(300));
					esp_restart();
					return true;
				}
// usbplay – stream currently playing USB file for browser Monitoring
				param = strstr(c, "usbplay");
				if (param != NULL) {
					serveUsbPlay(conn);
					return true;
				}
// usbfiles command  ?usbfiles  list playable files on the mounted USB drive as JSON
				param = strstr(c,"usbfiles") ;
				if (param != NULL) {
					char *list = inmalloc(1024);
					if (list == NULL) {
						ESP_LOGE(TAG," %s kmalloc fails","usbfiles");
						respOk(conn,"[]");
						return true;
					}
					if (usb_msc_list_files(list, 1024)) {
						respOk(conn,list);
					} else {
						respOk(conn,"[]");
					}
...
// file GET
			{
				if (strcmp(c, "/usbplay") == 0 || strcmp(c, "usbplay") == 0) {
					serveUsbPlay(conn);
					return true;
				}
				if(strlen(c) > 32) {
...
void serverclientTask(void *pvParams) {
	...
	// DRECLEN (1536 bytes) is by far the biggest single consumer of this
	// task's stack budget (see "stack" in servers.c). It used to live on
	// the stack as a plain array; putting it on the heap instead (PSRAM,
	// via kmalloc/inmalloc) frees that margin back for every request,
	// which matters now that handlePOST() has grown extra branches.
	char *buf = (char *)inmalloc(DRECLEN);
	bool result = true;
	...
	if (buf != NULL)
	{
		...
		{ // For now we assume max. RECLEN bytes for request
			...
			char* bend = NULL;
			bool isOtaPost = false;
			do {
				bend = strstr(buf, "\r\n\r\n");
				if (bend != NULL)
				{
					bend += 4;
					if (strstr(buf,"POST /ota") != NULL)
					{
						isOtaPost = true;
						break; // OTA has its own streaming receive path below
					}
...
//						printf ("Server: received more for end now: %d bytes\n", recbytes+recb);
			...
			} while ((bend == NULL) && !isOtaPost);
			if (isOtaPost)
			{
				uint32_t contentLength = (uint32_t)atol(strstr(buf, "Content-Length: ")+16);
				handleOtaUpload(client_sock, buf, recbytes, bend, contentLength);
				result = true; // done with this connection either way
			}
			else if (bend != NULL)
		...
		ESP_LOGE(TAG," %s kmalloc fails","buf");
		result = true; // nothing to clean up on the socket read side, just close it
	}
	if (result)
	{
		int err;
