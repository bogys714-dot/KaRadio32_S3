...
void draw(int i)
{
    ...
    switch (i) {
        case STATIONNAME:
		setfont(text);
        ucg_SetColori(&ucg,255,255,255); // цвет фона (белый)  
        ucg_DrawBox(&ucg,0,0,x,y/*-ucg_GetFontDescent(&ucg)*/);  
        ucg_SetColori(&ucg,0,0,0);   // цвет текста (чёрный для читаемости)
		if (lline[i] != NULL)
		{
...
////////////////////
// draw the USB file list screen (browse/select) — same layout as
// drawStationUcg, just a different title
void drawUsbListUcg(uint8_t mTscreen,char* snum,char* ddot)
{
...
{
	// FIX: was "char newstation[BUFLEN];" on the stack - task_lcd (which
	// calls this) only has a small stack budget; a static buffer costs
	// nothing per-call instead of BUFLEN bytes every time.
	static char newstation[BUFLEN];
...
void namesetUcg(char* ici)
{
	...
	// FIX: was "char nameseti[BUFLEN];" on the stack - see icy4Ucg above
	// for the same reasoning (task_lcd's stack is tight).
	static char nameseti[BUFLEN];
