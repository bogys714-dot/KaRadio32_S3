...
// USB flash-drive (MSC) playback enable/disable (applied at next boot,
// see usb_msc_init() call in app_main.c). Defaults to enabled (1) so
// existing installs that never touch this setting keep working exactly
// as before.
void option_get_usben(uint8_t *enca)
{
	...
	*enca = 1;
	...
	err = nvs_get_u8(hardware_handle, "O_USB_EN",(uint8_t *) &en);

	if (err != ESP_OK) ESP_LOGD(TAG,"oget_usben err 0x%x",err);
	else{
		if (en != 255) *enca = en;
	}
...
void option_set_usben(uint8_t enca)
{
	...
	if (open_partition(hardware, option_space,NVS_READWRITE,&hardware_handle)!= ESP_OK)
	{
		ESP_LOGD(TAG,"set_usben");
		return;
	}	
	
	err = nvs_set_u8(hardware_handle, "O_USB_EN",enca?1:0);
	if (err != ESP_OK) ESP_LOGD(TAG,"oset_usben err 0x%x",err);
...
void gpio_get_encoders(gpio_num_t *enca, gpio_num_t *encb, gpio_num_t *encbtn, gpio_num_t *enca1, gpio_num_t *encb1, gpio_num_t *encbtn1)
{
	...
	nvs_handle hardware_handle;
	// init default
	// NOTE: the previous code forced GPIO_NONE (disabling the encoder)
	// whenever bigSram() was true, because on some boards the OLD default
	// encoder pins (16/17) clashed with PSRAM/flash lines. The CURRENT
	// defaults (PIN_ENC0_A/B/BTN = GPIO4/GPIO3/GPIO8) do not use those
	// pins, so we always use the compiled-in defaults here and let the
	// NVS "hardware" partition below override them if the user has
	// explicitly configured different pins.
	*enca = PIN_ENC0_A;
	*encb= PIN_ENC0_B;
	*encbtn= PIN_ENC0_BTN;
	*enca1 = PIN_ENC1_A;
