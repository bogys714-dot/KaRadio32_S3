...
function mplay(){
	monitor = document.getElementById("audio");
	// If currently playing a USB file (name starts with "USB:"), stream from /usbplay
	var nameEl = document.getElementById("name");
	var isUsb = nameEl && (nameEl.innerHTML.indexOf("USB:") === 0 || nameEl.innerText.indexOf("USB:") === 0);
	var src;
	if (isUsb) {
		src = window.location.protocol + "//" + window.location.host + "/usbplay";
	} else if (urlmonitor && urlmonitor.endsWith("/")) {
		src = urlmonitor + ";";
	} else {
		src = urlmonitor;
	}
...
function hardware(valid) {
	...
	xhr.onreadystatechange = function() {
		if (xhr.readyState == 4 && xhr.status == 200) {	
			...
			if (document.getElementById("usbEnabled")) {
				document.getElementById("usbEnabled").checked = (arr['usben'] == "1");
			}
			if (document.getElementById("usbStatus")) {
				document.getElementById("usbStatus").innerText = (arr['usbready'] == "1") ? "флешка подключена" : "флешка не обнаружена";
			}
		}
	}
...
// Toggle whether the USB host stack starts at boot. Sends idlcd=91 so the
// hardware handler's display/audio block is skipped (91 = "no LCD" is
// already used elsewhere as a sentinel to leave that block untouched) --
// otherwise it would also rewrite lcd rotation to 0 on every USB toggle.
function toggleUsbEnabled() {
	var en = document.getElementById('usbEnabled').checked ? 1 : 0;
	if (!confirm((en? "Включить" : "Выключить") + " USB-хост? Изменение применится после перезагрузки устройства.")) {
		document.getElementById('usbEnabled').checked = !document.getElementById('usbEnabled').checked;
		return;
	}
	var i, coutput;
	for (i=0 ;i<6;i++) if (document.getElementById('output'+i).checked) break;
	...
	xhr2.open("POST","hardware",false);
	xhr2.setRequestHeader(content,ctype);
	xhr2.send("valid=1&coutput=" + coutput + "&idlcd=91&usben=" + en + "&");
}
// Ask the device for the list of playable files on the mounted USB drive.
function refreshUsbFiles() {
	var xhr2 = new XMLHttpRequest();
	xhr2.onreadystatechange = function() {
		if (xhr2.readyState == 4 && xhr2.status == 200) {
			var files = [];
			try { files = JSON.parse(xhr2.responseText); } catch(e) { files = []; }
			var sel = document.getElementById('usbFileList');
			sel.innerHTML = "";
			...
			if (files.length == 0) {
				var opt = document.createElement("option");
				opt.value = "";
				opt.text = "(файлы не найдены / флешка не подключена)";
				sel.appendChild(opt);
			}
		}
	}
	xhr2.open("GET","/?usbfiles",true);
	xhr2.send();
}
// Play the file currently selected in the USB file list.
function playUsbFile() {
	var sel = document.getElementById('usbFileList');
	if (sel.selectedIndex < 0) return;
	var file = sel.options[sel.selectedIndex].value;
	if (!file) return;
	var xhr2 = new XMLHttpRequest();
	xhr2.open("GET","/?usbfile=" + encodeURIComponent(file),true);
	xhr2.send();
	document.getElementById('usbPlayStatus').innerText = "Играет: " + file;
}
// Stop USB file playback.
function rebootDevice() {
	if (!confirm("Перезагрузить устройство сейчас?")) return;
	var xhr2 = new XMLHttpRequest();
	try {
		xhr2.open("GET","/?reboot",true);
		xhr2.send();
	} catch(e) {}
	alert("Перезагрузка...");
}
function stopUsbFile() {
	var xhr2 = new XMLHttpRequest();
	xhr2.open("GET","/?usbhalt",true);
	xhr2.send();
	document.getElementById('usbPlayStatus').innerText = "Остановлено";
}
...
function uploadFirmware()
{
	...
	var fb = document.getElementById('updatefb');
	if (!fileInput.files || fileInput.files.length === 0) {
		alert("Please choose a .bin firmware file first.");
		return;
	}
	var file = fileInput.files[0];
	if (!confirm("Upload \"" + file.name + "\" (" + file.size + " bytes) and start the update?\nThe device will reboot when done.")) return;
	fb.innerHTML = "Uploading 0%";
	...
	xhr.upload.onprogress = function(e) {
		if (e.lengthComputable) {
			...
			fb.innerHTML = "Uploading " + pct + "%";
		}
	};
	xhr.onload = function() {
		if (xhr.status == 200) {
			fb.innerHTML = "Update OK. Rebooting...";
			alert("Update successful. The device is rebooting.\nPlease reload the page in a few seconds.");
		} else {
			fb.innerHTML = "Update failed (status " + xhr.status + ")";
		}
	};
	xhr.onerror = function() {
		fb.innerHTML = "Upload error.";
	};
	xhr.send(file);
}
...
function applyDisplaySettings() {
    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState == 4 && xhr.status == 200) {
            alert("Настройки экрана и поворота отправлены!");
        }
    };
