...
static bool nec_rx_init()
{
	...
	// Проверка на невалидный GPIO (GPIO_NONE = 255 = -1 для gpio_num_t)
	if (ir == GPIO_NONE || ir == (gpio_num_t)GPIO_NONE || ir < 0 || ir > 48) {
		ESP_LOGD(NEC_TAG, "IR not configured (GPIO: %d)", ir);
		return false;
	}
	
	ESP_LOGI(NEC_TAG, "Initializing RMT on GPIO %d, channel %d", ir, RMT_RX_CHANNEL);
	...
	err = rmt_config(&rmt_rx);
	if (err != ESP_OK) {
		ESP_LOGE(NEC_TAG, "rmt_config failed: %s", esp_err_to_name(err));
		return false;
	}
	
	err = rmt_driver_install(rmt_rx.channel, 1000, 0);
	if (err != ESP_OK) {
		ESP_LOGE(NEC_TAG, "rmt_driver_install failed: %s", esp_err_to_name(err));
		return false;
	}
	
	ESP_LOGI(NEC_TAG, "RMT initialized successfully");
	return true;
}
